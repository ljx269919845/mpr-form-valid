...
domUtils.docWidth = refWin => {
  ...
  return Math.max(
    ...
    d.body[`scrollWidth`],
    domUtils[`viewportWidth`](d)
  );
};
domUtils.viewportWidht = win => {
...
domUtils.docHeight = refWin => {
  ...
  return Math.max(
    ...
    // quirks : documentElement.scrollHeight 最大等于可视窗口多一点？
    d.body[`scrollHeight`],
    domUtils[`viewportHeight`](d)
  );
};
domUtils.viewportHeight = win => {
  // pc browser includes scrollbar in window.innerWidth
...
function getWH(elem, name, extra) {
  ...
  if (borderBoxValueOrIsBorderBox) {
    const padding = extra === PADDING_INDEX ? -getPBMWidth(elem, ['border'], which) : getPBMWidth(elem, ['margin'], which);
...
function getWHIgnoreDisplay(elem, name?, extra?) {
  let val;
...
domUtils.outerWidth = (el, includeMargin) => {
  return el && getWHIgnoreDisplay(el, name, includeMargin ? MARGIN_INDEX : BORDER_INDEX);
};
domUtils.outerHeight = (el, includeMargin) => {
  return el && getWHIgnoreDisplay(el, name, includeMargin ? MARGIN_INDEX : BORDER_INDEX);
};
domUtils.width = (elem, val) => {
  if (val !== undefined) {
    if (elem) {
      const computedStyle = getComputedStyleX(elem);
      const isBorderBox = isBorderBoxFn(elem);
